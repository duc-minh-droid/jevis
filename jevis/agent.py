...
def _execute(steps: list[dict], log: list[str], window=None) -> tools.Snapshot | None:
    ...
    for index, step in enumerate(steps[:MAX_STEPS], 1):
        ...
        if name == "launch":
            ...
            _verify(step, snap, clause="require")
...
def run(instruction: str, window=None, verbose: bool = True) -> Result:
    started = time.time()
    ...
    if matched:
        ...
        for attempt in range(1, SKILL_ATTEMPTS + 1):
            ...
            result.log.clear()
            try:
                snap = _execute(result.steps, result.log, window)
...
def main() -> int:
    ...
    if len(sys.argv) < 2:
        print('usage: python -m jevis.agent "open notepad and write hello world"')
        return 2
